    ...
    def calc_mention_freq(self):
        ...
        for tweet in self.data['message']:
            if isinstance(tweet, str):
                if tweet.startswith('re: @'):
                    username = tweet.split('re:')[1].split()[0]
                    if username in user_rt_count.keys():
                        user_rt_count[username] += 1
                    else:
                        user_rt_count[username] = 1
        return sorted(user_rt_count.items(), key=lambda kv: kv[1], reverse=True)
...
def plot_bar_tweet_freq(x, xticklabels):
    ...
    myvecs = [
        x[x.index(max(x)):x.index(13)],
    ]
    myvecticks = [
        xticklabels[x.index(max(x)):x.index(13)],
    ]
    for vec, vecticklabels in zip(myvecs,myvecticks):
        size = len(vec)

        plt.ylabel('Number of RTs (re: @) Overall', fontsize=26)
